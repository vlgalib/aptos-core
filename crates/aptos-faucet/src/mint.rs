...
async fn process(service: &Service, params: MintParams) -> Result<Response> {
    let asked_amount = params.amount.ok_or_else(|| anyhow::format_err!("Mint amount must be provided"))?;
    let service_amount = service.maximum_amount.unwrap_or_else(|| asked_amount);
